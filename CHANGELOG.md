...
- Custom domain per project — added customDomain field on Project; PUT /api/projects/:id/custom-domain endpoint; second Traefik router added for custom domain on deploy; live restart when domain changes on running project; CustomDomain organism on project detail page
- Email notifications on deploy — nodemailer email service with SMTP config; deploy flow sends success/failure emails fire-and-forget to assigned users and team leaders/elders; emailNotifications preference on User (default true); PUT /api/auth/notifications endpoint; toggle on /profile page; graceful degradation when SMTP not configured
- Project transfer between teams — POST /api/projects/:id/transfer endpoint (owner/admin only); atomic transfer with slug collision resolution and ProjectAssignment cleanup; TransferProject organism with team select and confirm dialog on project detail page (danger zone)
